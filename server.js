...
passport.deserializeUser(async (user, done) => {
  let result = await db
    ...
    if (result != null) {
      delete result.password; // result가 null이나 undefined가 아닐 때만 실행
      //password 프로퍼티 삭제
    }
...
app.get("/chat/detail/:id", async (req, res) => {
  ...
  // 챗팅방의 대화내용
  // parentRoom가 처음 디비에서 객체생성 했을 때
  // ObjectId로 받을 것인지 그냥 아이디로 받을 것인지를 정해야하는데 
  let chat = await db.collection("chatMessage").find({ parentRoom : new ObjectId(id) }).toArray();
...
// Socket.IO 에서 세션 사용
// req 에 담겨있는 session 값들을 사용할 수 있따.
...
io.on('connection', (socket)=>{
  ...
  socket.on('message-send', async(data)=>{ //객체
    // db에 저장하는 역할
    const user = await db.collection('user').findOne({_id : new ObjectId(socket.request.session.passport.user.id)})
    ...
    // 서버에서 클라이언트로 데이터 보낼때
    // 특정한 방번호에 메시지 보낸다
    io.to(data.room).emit('msg-broadcast', {
      msg : data.msg, 
      date : data.date,
      who : user.userid
    }) 
